...
class KickChatWorker(QObject):
    ...
    def _scan_for_levels(
        ...
        *,
        ...
        if level_ids:
            if self._queue.punishment_system_enabled and not is_broadcaster:
                non_punished_ids = []
                for level_id in level_ids:
                    punished, count = self._queue.record_level_submission(requester, level_id, is_broadcaster=is_broadcaster)
                    if punished:
